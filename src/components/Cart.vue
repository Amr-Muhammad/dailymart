<template>
  <div class="mx-auto p-6 rounded-lg shadow-lg w-6/12">
    ...
    <div v-if="cart != null">
      <div v-for="(item, index) in cart" :key="index" class="flex items-center justify-between border-b border-gray-200 py-4">
          ...
          <div>
            ...
            <div class="flex items-center space-x-2 mt-2">
              ...
              <div v-if="!customQty[index]">
                <select v-model="selectedQty[index]" class="border border-gray-300 rounded p-1" @change="checkQty(index, item[0])">
                  <option v-for="qty in 10" :key="qty" :value="qty">{{ qty }}</option>
                  <option :value="11">10+</option>
                </select>
              </div>
              <div v-else>
                <input v-model.number="customQtyValue[index]" type="number" min="11" class="border border-gray-300 rounded p-1 w-16" @blur="updateCustomQty(index, item[0])" />
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  async updateQty(index, productId, quantity) {
    try {
      ...
      await axios.patch(`https://dailymart-5c550-default-rtdb.firebaseio.com/cart/${this.userId}/${productId}/.json`, {
        quantity: quantity
      });
