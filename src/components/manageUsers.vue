          ...
          <table id="table" class="text-center w-full bg-white border">
            ...
            <tbody>
              <tr v-for="user in users" :key="user[0]" class="border">
                ...
                <td class="px-6 py-4 text-sm text-gray-500" :class="user[1].planid == 'a1b2c3_subscription' ? 'text-green-500' : ''">
                  {{ user[1].planid ? 'Yes' : 'No' }}
                </td>
                <td class="px-6 py-4 text-sm text-gray-500">
                  <button v-if="user[1].planid" @click="viewUserProducts(user[0])" class="border text-black px-4 py-2 rounded hover:bg-violet-900 hover:text-white transition-all duration-300">View</button>
                  <p v-if="!user[1].planid">------</p>
                </td>
                <td class="px-6 py-4 text-sm" :class="user[1].orderStatus == 'Approved' ? 'text-green-700 font-semibold' : 'text-red-500 font-semibold'">
                ...
                <td class="py-4 text-sm text-gray-500">
                  <svg @click="deleteItem(user[0])" class="block mx-auto cursor-pointer" fill="#DB4444" width="25px" height="25px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" stroke="#DB4444">
