  ...
  <div class="container mx-auto px-8 py-10">
    ...
    <div class="card grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
      ...
      <div v-if="productD" class="flex w-full justify-center bg-[#F5F5F5] p-4 rounded-lg relative">
        ...
        <button title="Add To Wishlist" v-if="productD" class="text-red-500 hover:text-red-600 text-3xl absolute right-[25px] top-[25px]">
        ...
        <button title="Add to Weekly Order" v-if="productD && productD.availability > 0"
          ...
          <svg svg class="hover:scale-[1.1]"
              @click="addToWeeklyOrder(productId, productD, $event)"
      ...
      <div v-if="productD" class="flex flex-col ">
        ...
        <div class="flex space-x-0 md:space-x-2 my-4 flex-col md:flex-row items-start md:items-center">
          ...
          <div class="rating rating-md relative my-1">
              ...
              :key="star" 
              ref="inputs" 
              :checked="Math.round(allRates) === star" 
              type="radio" 
              :name="'rating-7'" 
              class="mask mask-star-2 bg-orange-400" 
              :class="{'opacity-[0.2]': allRates === 0}" 
              disabled 
            />
          </div>
        ...
        <div class="flex items-center relative my-4">
          ...
          <input :disabled="productD.availability <= 0" type="number" v-model="this.quantity" 
            :min="productD.availability === 0 ? 0 : 1" :max="productD.availability" class="border-y-2 border-solid h-8 border-gray-700 text-center focus:outline-none focus-within:outline-none">
          ...
          <button :disabled="this.quantity == productD.availability || productD.availability <= 0" @click="this.quantity++" class="w-8 h-8 bg-[#166534] text-white border-2 border-solid border-[#166534]">+</button>
        </div>
          ...
          <div class="flex items-center mb-2">
            <div class="rating rating-sm relative" >
              <input v-for="star in 5" 
                ...
                :name="'rating-' + index" 
                :value="star" 
                :checked="review[1].rating === star" 
                type="radio" 
                class="mask mask-star-2 bg-yellow-400 cursor-auto" 
                disabled />
            </div>
          </div>
    ...
    <div class="mt-10">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4">
        ...
        <div v-for="relatedPrd in relatedPrds" :key="relatedPrd[0]" class="text-center p-4 bg-[#F5F5F5] rounded-lg shadow relative">
