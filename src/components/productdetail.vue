<template>
  <div class="container mx-auto px-8 py-10">
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
      ...
      <div v-if="productD" class=" bg-[#F5F5F5]">
        <div class="flex w-64 h-full justify-center mx-auto p-4 rounded-lg">
          <img :src="productD.image_url" alt="Product Image" class="w-80 h-80">
        </div>
  ...
  <div class="rating rating-md relative">
    <button @click="showModal = true" class="w-full h-full z-10 absolute top-0 left-0"></button>
    <input ref="inputs" :checked="averageRating >= 1" type="radio" name="rating-7"
      class="mask mask-star-2" :style="{ backgroundColor: averageRating >= 1 ? '#FFA500' : '#87CEFA' }" disabled />
    <input ref="inputs" :checked="averageRating >= 2" type="radio" name="rating-7"
      class="mask mask-star-2" :style="{ backgroundColor: averageRating >= 2 ? '#FFA500' : '#87CEFA' }" disabled />
    <input ref="inputs" :checked="averageRating >= 3" type="radio" name="rating-7"
      class="mask mask-star-2" :style="{ backgroundColor: averageRating >= 3 ? '#FFA500' : '#87CEFA' }" disabled />
    <input ref="inputs" :checked="averageRating >= 4" type="radio" name="rating-7"
      class="mask mask-star-2" :style="{ backgroundColor: averageRating >= 4 ? '#FFA500' : '#87CEFA' }" disabled />
    <input ref="inputs" :checked="averageRating >= 5" type="radio" name="rating-7"
      class="mask mask-star-2" :style="{ backgroundColor: averageRating >= 5 ? '#FFA500' : '#87CEFA' }" disabled />
  </div>
</div>
        ...
        <div class="flex items-center space-x-2">
          <button @click="addToCart"
            class="bg-green-600 hover:bg-[#166534] text-white px-4 py-2 text-sm rounded w-full">
            Add to Cart
          </button>
          <button @click="addToWishlist" class="text-red-500 hover:text-red-600 text-3xl">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
              stroke="currentColor"
              class="size-8 cursor-pointer hover:fill-[#DB4444] transition-all duration-1000 ms-auto text-[#DB4444] block mx-auto border-2 border-solid border-[#DB4444] rounded p-1">
        ...
        <!-- <div>
          <button @click="showModal = true" class="mt-4 bg-[#db4444] hover:bg-[#0a1e1e] text-white font-semibold p-3 rounded-lg shadow-md transition duration-200 w-full">
            Rate Product
          </button>
        </div> -->
      </div>
      ...
      <div class="space-y-4">
        <div v-for="review in getReviews" :key="review" class="border-t pt-4">
          <div class="flex items-center mb-2">
            ...
            <p class="font-semibold">Menna</p>
          </div>
          <div class="flex items-center mb-2">
            <span class="text-yellow-400">
              <span v-for="star in review[1].rating" :key="star">★</span>
            </span>
          </div>
...
export default {
  ...
  try {
    // التحقق من المدخلات
    if (!this.rating) {
      alert('يرجى تقديم تقييم.'); 
      return;
    }
    
    if (this.comment && this.comment.length < 2) {
      alert('يرجى كتابة تعليق يتكون من 5 أحرف على الأقل.');
      return;
    }
    ...
    await axios.post("https://dailymart-5c550-default-rtdb.firebaseio.com/comments.json", newReview);
    
    // تحديث عدد المراجعات مباشرة بعد الإضافة
    this.totalReviews++;
    ...
    // جلب المراجعات وتحديث الواجهة
    await this.fetchReviews();
    await this.getProductRate();
    await this.displayReviews();  // تأكد من تحديث المراجعات المعروضة
...
fetchReviews() {
  // دالة لجلب المراجعات من الخادم وتحديث الحالة
  axios.get("https://dailymart-5c550-default-rtdb.firebaseio.com/comments.json")
    .then(response => {
      this.reviews = response.data;
    })
    .catch(err => {
      console.error('فشل في جلب المراجعات:', err);
    });
},
    async displayReviews() {
      try {
        ...
        // console.log("Displayed Reviews:", this.getReviews);
      } catch (err) {
    ...
    async getProductDetails() {
      try {
        ...
        this.productD = Object.entries(response).filter(item => item[0] === this.productId)[0][1];

        this.relatedPrds = Object.entries(response).filter(item => item[1].catId === this.productD.catId);
    ...
    async getProductRate() {
      try {
        ...
        const totalRating = this.commentD.reduce((sum, comment) => sum + comment[1].rating, 0);
        const averageRating = (totalRating / this.commentD.length) || 0;

        console.log("Average Rating:", averageRating);

        //   تحديث قاعدة البيانات بهذا المتوسط
        await this.updateAverageRatingInDB(averageRating);
      } catch (err) {
        console.log('Cannot Fetch Data', err);
      }
    },
    
    async updateAverageRatingInDB(averageRating) {
      try {
        ...
        console.log('Average rating updated successfully!', averageRating); // رسالة تأكيد

      } catch (err) {
        console.error('Error updating average rating in the database:', err);
      }
    },
